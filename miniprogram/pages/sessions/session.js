...
Page({
    ...
    updateStatusDisplay: function () {
        ...
        var targetText = '';
        if (session.target.type === session_1.SessionTargetType.RunCount) {
            targetText = 'Run: ' + session.runCount + ' / ' + session.target.value;
        }
        else if (session.target.type === session_1.SessionTargetType.Time) {
    ...
    onTogglePause: function () {
        ...
        if (session.status === session_1.SessionStatus.Active) {
            session.status = session_1.SessionStatus.Paused;
        }
        else {
            session.status = session_1.SessionStatus.Active;
        }
        session.lastUpdate = Date.now();
        this.setData({ session: session });
        SessionStorageService.saveCurrentSession(session);
