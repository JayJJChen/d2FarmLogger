...
var ItemStorageService = {
    ...
    getAllItems: function () {
        try {
            ...
            // 合并内置物品和用户物品
            var allItems = item_library_1.BUILT_IN_ITEMS.concat(userItems);
            return allItems;
        }
        catch (error) {
            console.error('获取物品数据失败:', error);
            return item_library_1.BUILT_IN_ITEMS; // 如果获取失败，至少返回内置物品
        }
    },
    /**
     * 创建新物品
     ...
     */
    createItem: function (name, category) {
        try {
            // 验证参数
            if (!name || !name.trim()) {
                wx.showToast({
            ...
            // 检查分类是否支持用户添加
            var userCategories = item_library_1.ITEM_CATEGORIES.slice(0, 5); // 只支持前5个分类：暗金、套装、稀有、魔法、底材
     ...
     * @returns {boolean} 更新是否成功
     */
    incrementUsageCount: function (itemId) {
        try {
            var allItems = this.getAllItems();
            var itemFound = false;
            // 在内置物品中查找
            for (var i = 0; i < item_library_1.BUILT_IN_ITEMS.length; i++) {
                if (item_library_1.BUILT_IN_ITEMS[i].id === itemId) {
                    item_library_1.BUILT_IN_ITEMS[i].usageCount++;
                    item_library_1.BUILT_IN_ITEMS[i].updateTime = Date.now();
                    itemFound = true;
                    break;
                }
            }
