...
var SceneStorageService = {
  ...
  initializeDefaultScenes: function () {
    try {
      ...
      var scenes = defaultScenes.map(function (sceneData) {
        return this._createSceneObject(sceneData);
      }.bind(this));
  ...
  createScene: function (name, description) {
    try {
      // 验证参数
      if (!this.validateSceneName(name)) {
        wx.showToast({
          title: '场景名称无效（1-50字符）',
          icon: 'none'
        });
      ...
      if (description !== undefined && !this.validateSceneDescription(description)) {
        wx.showToast({
      ...
      // 创建新场景对象
      var newScene = this._createSceneObject({
  ...
  updateScene: function (sceneId, updates) {
    try {
      ...
      if (updates.name !== undefined) {
        if (!this.validateSceneName(updates.name)) {
          wx.showToast({
            title: '场景名称无效（1-50字符）',
            icon: 'none'
          });
      ...
      if (updates.description !== undefined) {
        if (!this.validateSceneDescription(updates.description)) {
          wx.showToast({
            title: '场景描述过长（最多200字符）',
            icon: 'none'
          });
   ...
   * @returns {SceneFlow|null} 场景流程对象或null
   */
  ...
  createSceneFlow: function (name, sceneIds, description) {
    try {
      // 验证参数
      if (!this.validateSceneFlowName(name)) {
        wx.showToast({
          title: '流程名称无效（1-50字符）',
          icon: 'none'
        });
      ...
      if (!this.validateSceneIds(sceneIds)) {
        wx.showToast({
      ...
      if (description !== undefined && !this.validateSceneFlowDescription(description)) {
        wx.showToast({
      ...
      // 创建新场景流程对象
      var newSceneFlow = this._createSceneFlowObject({
        id: StorageUtils.generateUniqueId(),
        name: trimmedName,
        description: description ? description.trim() : undefined,
        sceneIds: sceneIds,
  ...
  updateSceneFlow: function (flowId, updates) {
    try {
      ...
      // 验证更新数据
      if (updates.name !== undefined) {
        if (!this.validateSceneFlowName(updates.name)) {
          wx.showToast({
            title: '流程名称无效（1-50字符）',
            icon: 'none'
          });
      ...
      if (updates.description !== undefined) {
        if (!this.validateSceneFlowDescription(updates.description)) {
          wx.showToast({
            title: '流程描述过长（最多200字符）',
            icon: 'none'
          });
      ...
      if (updates.sceneIds !== undefined) {
        if (!this.validateSceneIds(updates.sceneIds)) {
          wx.showToast({
            title: '场景列表无效（1-20个场景）',
            icon: 'none'
          });
   ...
   */
  validateSceneName: function (name) {
   ...
   * @returns {boolean} 是否有效
   */
  validateSceneDescription: function (description) {
   ...
   * @returns {boolean} 是否有效
   */
  validateSceneFlowName: function (name) {
    if (!name || typeof name !== 'string') {
      return false;
    }
    if (name.length < 1 || name.length > 50) {
  ...
  /**
   * 验证场景流程描述
   * @param {string} description - 场景流程描述
   * @returns {boolean} 是否有效
   */
  validateSceneFlowDescription: function (description) {
    if (description === undefined || description === null) {
      return true; // 描述是可选的
    }
    if (typeof description !== 'string') {
   ...
   * @returns {boolean} 是否有效
   */
  validateSceneIds: function (sceneIds) {
   ...
   */
  _createSceneObject: function (data) {
    var scene = {
      id: data.id || '',
      name: data.name || '',
  ...
  /**
   * 创建场景流程对象
   * @param {Object} data - 场景流程数据
   * @returns {Object} 完整的场景流程对象
   */
  _createSceneFlowObject: function (data) {
    var sceneFlow = {
      id: data.id || '',
      name: data.name || '',
...
module.exports = {
  SceneStorageService: SceneStorageService,
