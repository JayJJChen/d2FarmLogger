...
Component({
  properties: {
    ...
    route: {
      type: Object,
      value: null,
      observer: function(newVal: SceneFlow) {
  ...
  methods: {
    ...
    // 添加场景到选择列表
    addSceneToSelection(sceneId: string) {
      var availableScenes = this.data.availableScenes
      var formData = this.data.formData
    ...
    // 从选择列表移除场景
    removeSceneFromSelection(sceneId: string) {
      var availableScenes = this.data.availableScenes
      var formData = this.data.formData
    ...
    moveSceneUp(e: any) {
      ...
      if (currentIndex <= 0) return // 已经在第一个位置
    ...
    moveSceneDown(e: any) {
      ...
      if (currentIndex >= formData.sceneIds.length - 1 || currentIndex === -1) return // 已经在最后一个位置
    ...
    // 更新场景顺序
    updateSceneOrders(sceneIds: string[]) {
      var availableScenes = this.data.availableScenes
    ...
    // 根据场景ID获取场景名称
    getSceneNamesByIds(sceneIds: string[]): string[] {
      try {
        var sceneNames = []
        for (var i = 0; i < sceneIds.length; i++) {
          var sceneId = sceneIds[i]
